...
async function injectDashboard() {
  ...
  wrapper.style.isolation = 'isolate'; // Traps z-index: -1 blobs inside
...
async function uploadImageFB(file) {
  ...
  formData.append("upload_id", "jsc_c_" + Math.floor(Math.random()*1000));
...
function initDashboardLogic(shadow) {
  ...
  function addLog(message, type = 'info') {
    ...
    // Using simple text styling based on type since CSS classes might not be fully defined
    let color = '#e5e7eb';
  ...
  els.btnStartPost.addEventListener('click', async () => {
    ...
    const imageFile = els.postImage.files[0];
    ...
    const imageMode = els.imageMode ? els.imageMode.value : 'all';
    ...
    for (let i = 0; i < targetGroups.length; i++) {
      ...
      if (globalImageIds.length > 0) {
        if (imageMode === 'sequential') {
          ...
          // Default to random to prevent FB from dropping the post if multiple attachments are passed
